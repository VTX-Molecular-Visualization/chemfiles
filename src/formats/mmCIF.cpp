...
template <>
const FormatMetadata &chemfiles::format_metadata<mmCIFFormat>()
{
		...
		if (!in_loop)
		{
			...
			if (line_split[0] == "_struct.title")
			{
				...
				else
				{
					...
					{
						name_ = std::string(line.substr(1, line.size() - 2));
					}
					else // Single line
					{
						name_ = std::string(line);
					}
				}
			}
		}
...
void mmCIFFormat::init_atom_site()
{
	...
	frame_positions_.push_back(reader_meta_data_.position);
...
void mmCIFFormat::addCategory(const std::string &category_name, bool is_loop)
{
	...
	else
	{
		...
		do
		{
			...
			{
				auto label = std::string(trim(line_split[0]).substr(category_name_length));
...
void mmCIFFormat::fill_loop_properties(const std::string &category_name, std::map<std::string, size_t> &property_map)
{
	...
	do
	{
		...
		if (line.find(category_name) != std::string::npos)
		{
			auto atom_label = std::string(trim(line).substr(category_name_length));
...
void mmCIFFormat::read_property_line(std::vector<std::string> &properties)
{
	...
	while (current < line.size())
	{
		...
		if (currentChar == '\'')
		{
			if (inString)
			{
				properties.emplace_back(std::string(line.substr(wordStart, current - wordStart)));
				inString = false;
			}
			else
			{
...
void mmCIFFormat::read_at(const size_t index, Frame &frame)
{
	...
	{ // The following map operations can be moved to the constructor
		...
		auto startSectionPos = steps_positions_[current_step_];
		auto isLastStep = current_step_ >= (steps_positions_.size() - 1);
		auto endSectionPos = isLastStep ? atom_site.position_data_end : steps_positions_[current_step_ + 1];

		file_.seekpos(startSectionPos);
		reader_meta_data_.position = startSectionPos;
		...
		while (true)
		{
			...
			if (file_.tellpos() > endSectionPos)
			{
				break;
			}
			...
			std::string_view atomName = line_split[label_atom_id->second];
			...
			const std::string symbolStr = std::string(line_split[type_symbol]);
			Atom atom(std::string(atomName), symbolStr);
	...
	void mmCIFFormat::read_inline_property(const std::vector<std::string_view> &line_split, std::string &data)
	{
		if (line_split.size() == 2)
		{
			data = std::string(line_split[1]);
		}
		else
		{
			auto line = file_.readline();
	...
	void mmCIFFormat::fill_assembly()
	{
		...
		if (pdbx_struct_assembly_gen.is_loop)
		{
			...
			do
			{
				...
				std::string_view operation_expression;
				std::string_view targets_str;
				...
				build_assembly_generators(std::string(assembly_id), operation_expression, targets);
				line = file_.readline();
			} while (file_.tellpos() < pdbx_struct_assembly_gen.position_data_end);
		}
		else
	...
	void mmCIFFormat::fill_assembly_operations()
	{
		...
		else
		{
			...
			assembly_.assembly_operations.emplace(std::string(id_value), AssemblyOperation(translation, rotation));
		}
	}
	...
	std::string mmCIFFormat::read_multi_line()
	{
		...
		while (!file_.eof())
		{
			result += std::string(line.substr(1));
	...
	void mmCIFFormat::fill_assembly_targets_vector(const std::string_view &targets_str, std::set<std::string> &targets)
	{
		std::string_view target_group_sv;
	...
	void mmCIFFormat::build_assembly_generators(const std::string &assembly_id,
												const std::string_view &operation_expression,
												const std::set<std::string> &targets)
	{
		...
		if ((special_symbol_position = operation_expression.find_last_of('(')) != std::string::npos)
		{
			...
			auto outer_expression = std::string(lhs_expression) + std::string(rhs_expression);
		...
		else
		{
			...
			assemblyGenerator.operations.emplace_back(std::string(operation_expression));
