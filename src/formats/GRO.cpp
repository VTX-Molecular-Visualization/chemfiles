...
void GROFormat::read_next(Frame &frame)
{
	...
	for (size_t i = 0; i < natoms; i++)
	{
		...
		if (*resid != previousResidueIndexRead)
		{
			if (currentResidue != nullopt)
			{
				frame.add_residue(*currentResidue);
			}

			if (*resid < previousResidueIndexRead)
			{
				if (*resid == 0 && (previousResidueIndexRead == GRO_INDEX_MAX))
				{
					residIndexOffset += GRO_INDEX_MAX + 1;
				}
				else
				{
					chainIndex++;
					chainName = get_chain_name_from_index(chainIndex);
				}
			}

			const int64_t residueIndexToStore = *resid + residIndexOffset;

			currentResidue = Residue(resname, residueIndexToStore);
			currentResidue->set("chainname", chainName);
			currentResidue->set("chainid", chainName);

			previousResidueIndexRead = *resid;
		}

		currentResidue->add_atom(frame.size() - 1);
	}
...
std::string GROFormat::get_chain_name_from_index(const int chain_index)
{
	int current_chain_index = chain_index;
	size_t strSize = 1;
	while (current_chain_index >= 26)
	{
		current_chain_index /= 26;
		strSize++;
	}

	std::string chainStr = std::string(strSize, 'A');
	current_chain_index = chain_index;
	for (size_t i = 0; i < strSize; i++)
	{
		const char chainCharIndex = current_chain_index % 26;
		const char chainDisplayedChar = 'A' + chainCharIndex;
		chainStr[strSize - i - 1] = chainDisplayedChar;
...
void GROFormat::write_next(const Frame &frame)
{
	...
	for (size_t i = 0; i < frame.size(); i++)
	{
		...
		if (residue && residue->id())
		{
			...
			{
				warning(
					"GRO writer", "the residue id '{}' should not be negative or zero, treating it as blank", value);
				value = max_resid++;
...
optional<uint64_t> GROFormat::forward()
{
	...
	for (size_t i = 0; i < n_atoms + 1; i++)
	{
		if (file_.eof())
		{
			throw format_error("not enough lines in '{}' for GRO format", file_.path());
		}
		file_.readline();
	}
